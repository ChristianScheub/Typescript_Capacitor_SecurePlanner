import { encryptAndStore, decryptFromStorage } from "../../encryptionEngine/encryptionEngine";
...
export const loadAllToDoLists = async (encryptionKey: string): Promise<[ToDoList, string][]> => {
  try {
...
export const loadToDoList = async (noteId: string, encryptionKey: string): Promise<ToDoList | null> => {
      try {
...
export const saveToDoList = async (note: ToDoList, encryptionKey: string, noteId?: string): Promise<void> => {
    try {
